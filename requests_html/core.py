...
from typing import Set, Union, List, MutableMapping, Optional, Any, Dict
import aiohttp
import pyppeteer
import requests  # ✅ Ampiana mazava tsara
import shutil
...
class BaseSession(requests.Session):
    ...
    async def browser(self):
        if not hasattr(self, "_browser"):
            chromium_path = shutil.which("chromium-browser") or shutil.which("chromium") or shutil.which("chrome")
            if not chromium_path:
              exit(f"chromium not found\n  pkg install chromium -y")
            self._browser = await pyppeteer.launch(executablePath=chromium_path, headless=True, args=self.__browser_args)
...
# core.py - Ampio io class io

class AsyncRequests:
    """
    Async version of requests - manao get, post, etc. miaraka amin'ny async/await
    """
    ...
    async def _get_session(self) -> aiohttp.ClientSession:
        """Mamerina ny session raha misy, na mamorona vaovao."""
    ...
    async def post(self, url: str, data: Any = None, json: Any = None, **kwargs) -> 'AsyncResponse':
        """POST request asynchronous."""
        session = await self._get_session()
        response = await session.post(url, data=data, json=json, **kwargs)
        return AsyncResponse(response)
    
    async def put(self, url: str, data: Any = None, **kwargs) -> 'AsyncResponse':
        """PUT request asynchronous."""
        session = await self._get_session()
        response = await session.put(url, data=data, **kwargs)
        return AsyncResponse(response)
    
    async def delete(self, url: str, **kwargs) -> 'AsyncResponse':
        """DELETE request asynchronous."""
        session = await self._get_session()
        response = await session.delete(url, **kwargs)
        return AsyncResponse(response)
    
    async def patch(self, url: str, data: Any = None, **kwargs) -> 'AsyncResponse':
        """PATCH request asynchronous."""
        session = await self._get_session()
        response = await session.patch(url, data=data, **kwargs)
        return AsyncResponse(response)
    
    async def head(self, url: str, **kwargs) -> 'AsyncResponse':
        """HEAD request asynchronous."""
        session = await self._get_session()
        response = await session.head(url, **kwargs)
        return AsyncResponse(response)
    
    async def options(self, url: str, **kwargs) -> 'AsyncResponse':
        """OPTIONS request asynchronous."""
        session = await self._get_session()
        response = await session.options(url, **kwargs)
        return AsyncResponse(response)
...
class AsyncResponse:
    """Async response wrapper - mitovy amin'ny requests.Response saingy async."""
    ...
    async def text(self) -> str:
        """Mamerina ny text amin'ny response."""
    ...
    async def content(self) -> bytes:
        """Mamerina ny content (bytes) amin'ny response."""
        if self._content is None:
            self._content = await self._response.read()
        return self._content
    
    async def json(self) -> Any:
        """Mamerina ny JSON response."""
